    ...
    public static void main(String[] args) throws Exception {
        ...
        while (inFile.hasNextLine()) {
            ...
            for (int j = 0; j < s.length; j += 3) {
                ...
                g.addEdge(Integer.parseInt(s[j].trim()), Integer.parseInt(s[j + 1].trim()),
                        Double.parseDouble(s[j + 2].trim()));
                g.addEdge(Integer.parseInt(s[j + 1].trim()), Integer.parseInt(s[j].trim()),
                        Double.parseDouble(s[j + 2].trim()));
            }
        }
        ...
        //System.out.println("The total weight in the tree is " + tree.getTotalWeight());
        
        //tree.printTree();
        UnweightedGraph.SearchTree tree2 = g.getShortestPath(source);
